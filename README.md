...
or
```{bash}
python Anchor_Clustering_PostVJ.py -F input_file -d distance_type -t clustering_threshold -s vjgrouping -b birch_radius -f fraction_data -z size_threshold -m minimum_distance_ratio -p population_size -r random_material_rate -l linkage_method
```
...
## Example
```{bash}
python anchor_clustering_pipeline.py -F input_file.tsv -f nt -d norm_hd -t 0.15 -s vj -b 0.5 -p 1 -z 1000 -m 0.6 -l single
```

## Output
The output will be a CSV file with the prefix AC_Pre_ or AC_Post_ followed by various parameters, such as AC_Pre_VJ_0.6_1000_50_0.5_0.1_1000_0.12_input_file.csv. The file will contain the original input file columns along with their cluster assignments.
